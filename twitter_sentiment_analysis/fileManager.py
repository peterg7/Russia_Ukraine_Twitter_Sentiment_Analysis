...
EXCLUDE = r'|'.join(['\.ipynb$', '\.py$', '\.DS_Store$'])
...
def cleanDataOutput(keep=1, strip_most_recent=False, targets=[], purge=False, verbose=False):
    ...
    target_dirs = { 
        ...
        'notebooks/pipeline_2/': ['models/', 'data/transformed/', 'data/embeddings/']
    }
    ...
    def parseAndStage(dir_path):
        ...
        return staged_paths, filenames

    # TODO: need second feature of moving files (or just keep referencing them from pipeline 2?)
    # def moveToNext(filenames):
    ...
    for p in directory_paths:
        paths, names = parseAndStage(p)
        collected_paths.extend([x for x in paths if not re.search(EXCLUDE, x)])
