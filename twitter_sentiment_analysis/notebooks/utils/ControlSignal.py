...
class ControlSignal:
    ...
    class FLAGS(IntEnum):
        ...
        USER_INPUT = 14
        INVALID_LOCATION = 15
        MISSING_NECCESSARY = 16
        MISSING_IMPORT_LOC = 17
        MISSING_EXPORT_LOC = 18
        MISSING_OPTIONAL_ARGS = 19
        UNKNOWN = 99
        def __str__(self):
...
def processSignals(signals, generated_files=[], terminate=False):
    ...
    for sig in sorted(std_signals, reverse=True):
        print(sig)

    if abort_signals:
        for sig in sorted(abort_signals, reverse=True):
            print(sig)

        # Process aborted. Clean up...
        staged_paths = [path for path in generated_files.values() if os.path.isfile(path)]
        if staged_paths:
            print('Cleaning...')
